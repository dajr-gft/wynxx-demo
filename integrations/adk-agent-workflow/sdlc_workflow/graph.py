...
"""Self-contained graph runtime for the OFFLINE stub workflow.

A compact, deterministic runtime (``node`` decorator + ``Workflow`` runner) that
executes the article's fan-out/fan-in topology with **no external dependencies**,
so the example runs in CI and offline. It mirrors the *shape* of a graph runtime
for teaching purposes — it is not ADK itself.

The genuine ADK 2.0 orchestration lives in ``agent.py`` real mode, built from
``google.adk.agents`` (``SequentialAgent`` / ``ParallelAgent``) and deployed to
Vertex AI Agent Engine. (ADK 2.0 also ships a lower-level graph API under
``google.adk.workflow`` — singular — with ``Node`` / ``Edge`` / ``JoinNode``;
this module is intentionally simpler and dependency-free for the stub path.)
"""
...
# The offline path always uses this self-contained runtime; the real ADK
# orchestration is selected in agent.py when SDLC_MODE=real.
USING_ADK = False
...
class Workflow:
    ...
    def run(
        ...
        ``False`` rejects the gate and the node body is skipped. When no approver
        is supplied, gates auto-approve (and say so in the state).
        """
        ...
        while len(completed) < total:
            ...
            # Deterministic order; in the ADK runtime independent nodes run concurrently.
